...
class Preprocess():
    def __init__(self, image_name_path, images_path):
        self.image_name_path = image_name_path
        self.images_path = images_path
        self.detector = MTCNN()

    def run(self):
        with open(self.image_name_path, 'r') as f:
            image_names = [line.strip() for line in f]
        
        for i in image_names:
            img_path = self.images_path + f'/{i}'
            image = cv2.imread(img_path)
            ...
            cv2.normalize(cropped_img, cropped_img, 0, 255, cv2.NORM_MINMAX)
            gimage = cv2.equalizeHist(cropped_img)
            cv2.imwrite(f"C:/Code/461_data/GENKI-R2009a/Subsets/preprocessed/{i}", gimage)
